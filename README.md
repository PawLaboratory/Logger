...
- `Short` – single letter (D, I, W, E)
- `Medium` – three letters (DBG, INF, WRN, ERR)
- `Standard` – full names (DEBUG, INFO, WARN, ERROR)
...
- `{timestamp}` – formatted timestamp (using `DateFormat`)
- `{level}` – colored level text (brackets and color included)
